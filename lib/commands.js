...
import { notification, options as _options, getCssFilePaths, getProjectRootPathByFilePath } from './atom'
import { getHighlightJsStylePathByName } from './api/filesystem'
...
const convert = async (path, type) => {
  ...
  try {
    ...
    const convertedFilePath = await _convert(path, type, options, cssFilePaths)
