...
import { insertOptionsToFile, mergeOptionsWithFileOptions } from './api/atom/customOptions'
...
const commandConvert = async (target, type) => {
  try {
    ...
    if (dependencyStatus) {
      const markdownPath = await getEditorFilePath(target)
      const _options = await mergeOptionsWithFileOptions(markdownPath, options())
