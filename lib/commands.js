...
import { getHighlightJsStylePathByName, getFileDirectory, getDefaultExportFilePath } from './api/filesystem'
...
const convert = async (path, type) => {
  ...
  const destinationPath = getDefaultExportFilePath(path, type)
  const savePath = showSaveDialog(dialogTitle, destinationPath, dialogTitle, type) || destinationPath
  try {
