...
export const downloadChromiumNotification = async () => {
  let ui
  let success = false
  let detail = ''
  let description = ''
  ...
  const download = ({ target }) => {
    ...
    if (started === 0) {
      ...
      const progressCallback = (downloadedBytes, totalBytes) => {
        progressValueElement.innerHTML = `${formatBytes(downloadedBytes, 2)} / ${formatBytes(totalBytes, 2)}`
  ...
  description += `<span class="inline-block highlight-info"><span id="${progressValueId}">0/0</span> bytes</span>`
  description += `</div>`
