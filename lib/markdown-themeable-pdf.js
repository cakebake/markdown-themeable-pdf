...
module.exports = {
    config: {
        ...
        customStylesPath: {
            ...
            description: 'You can use this stylesheet file to customize everything.',
            order: 150
        },
        enableCustomHeader: {
            type: 'boolean',
        ...
        customHeaderPath: {
            type: 'string',
            default: path.join(atom.config.configDirPath, 'markdown-themeable-pdf', 'header.js'),
            description: 'You can use this javascript file to customize the document header.',
            order: 170
        },
        enableCustomFooter: {
            type: 'boolean',
        ...
        customFooterPath: {
            type: 'string',
            default: path.join(atom.config.configDirPath, 'markdown-themeable-pdf', 'footer.js'),
            description: 'You can use this javascript file to customize the document footer.',
            order: 190
        }
    },
    subscriptions: null,
    activate: function () {
        ...
        var customTemplatesSource = path.resolve(__dirname, '../templates');
    ...
    convertFile: function (filePath, encoding) {
        ...
        fs.readFile(filePath, encoding, function (err, markdown) {
            ...
            cssFile = path.normalize(atom.config.get('markdown-themeable-pdf.customStylesPath'));
            try {
                cssStyles += fs.readFileSync(cssFile, encoding) + '\n';
            } catch (err) {
                atom.notifications.addWarning('Stylesheet ' + cssFile + ' not found');
            ...
            if (exportType == 'html') {
                ...
                var customHeader = function () {
                    if (!atom.config.get('markdown-themeable-pdf.enableCustomHeader'))
                        return;

                    var obj, setting = path.normalize(atom.config.get('markdown-themeable-pdf.customHeaderPath'));
                    try {
                        obj = require(setting)();
                        if (typeof obj !== 'object')
                            return;

                        obj.contents = '<header id="page-header" class="meta">' + obj.contents + '</header>';

                        return obj;
                    } catch (err) {
                        atom.notifications.addWarning('Could not find custom header ' + setting);
                        console.error(err);
                        return;
                    }
                };
                var customFooter = function () {
                    if (!atom.config.get('markdown-themeable-pdf.enableCustomFooter'))
                        return;

                    var obj, setting = path.normalize(atom.config.get('markdown-themeable-pdf.customFooterPath'));
                    try {
                        obj = require(setting)();
                        if (typeof obj !== 'object')
                            return;

                        obj.contents = '<footer id="page-footer" class="meta">' + obj.contents + '</footer>';

                        return obj;
                    } catch (err) {
                        atom.notifications.addWarning('Could not find custom footer ' + setting);
                        console.error(err);
                        return;
                    }
                };
