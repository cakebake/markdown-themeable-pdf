...
module.exports = {
    ...
    activate: function () {
        ...
        var chp = atom.config.get('markdown-themeable-pdf.customHeaderPath'), chpExists = fs.existsSync(chp);
        fs.createFile(chp, function(err) {
            if (err) {
                atom.notifications.addError('Could not create custom header path: ' + err.message);
            } else if (!chpExists) {
                fs.writeFile(chp, '/**\n * Your markdown-themeable-pdf custom header\n */\n');
            }
        });
        var cfp = atom.config.get('markdown-themeable-pdf.customFooterPath'), cfpExists = fs.existsSync(cfp);
        fs.createFile(cfp, function(err) {
            if (err) {
                atom.notifications.addError('Could not create custom footer path: ' + err.message);
            } else if (!cfpExists) {
        ...
        fs.readFile(filePath, encoding, function (err, markdown) {
            ...
            if (exportType == 'html') {
                ...
                var customHeader = function () {
                    obj = require(atom.config.get('markdown-themeable-pdf.customHeaderPath'));
                ...
                var customFooter = function () {
                    obj = require(atom.config.get('markdown-themeable-pdf.customFooterPath'));
