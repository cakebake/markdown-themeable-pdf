...
import { getHighlightJsStyles } from './theme/highlightJs'
import { getBootswatchThemes } from './theme/bootswatch'
...
export const config = () => {
  ...
  getHighlightJsStyles().forEach((value) => {
    codeHighlightingTheme.push({ value, description: startCase(replace(value, '.css', '')) })
  })
  const theme = []
  getBootswatchThemes().forEach((value) => {
    theme.push({ value, description: startCase(value) })
  })
  return {
    imageExportFileType: {
      type: 'string',
    ...
    theme: {
      ...
      default: 'litera',
      enum: theme,
      description: 'Theme preview: https://bootswatch.com/',
      order: 145
    },
    customStylesPath: {
      type: 'string',
    ...
    downloadChromiumOnActivation: {
      type: 'boolean',
      default: true,
      description: 'Prompt for chromium download on atom startup or package update',
      order: 220
    }
  }
}
