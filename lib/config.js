...
import { join as implode } from 'lodash'
...
export const config = (highlightJsStyles = []) => {
  return {
    ...
    width: {
      ...
      type: 'string',
      default: '',
      description: `Overrides Papersize Format if set. Supported units: ${implode(SUPPORTED_UNITS, ', ')} (E.g. 10.5in)`,
      order: 25
    },
    height: {
      ...
      type: 'string',
      default: '',
      description: `Overrides Papersize Format if set. Supported units: ${implode(SUPPORTED_UNITS, ', ')} (E.g. 8in)`,
      order: 26
    },
    pageBorder: {
      ...
      type: 'string',
      default: '1cm',
      description: `Available only for export as pdf, jpg, png. Supported units: ${implode(SUPPORTED_UNITS, ', ')}`,
      order: 30
    },
    orientation: {
