...
import { getUserAtomPath } from './atom'
...
export const config = (highlightJsStyles = []) => {
  ...
  return {
    ...
    customHeaderPath: {
      ...
      description: 'You can use this javascript file to customize the document header. The path is relative to Atom config directory <code>' + getUserAtomPath() + '</code>. This relative path can also be used inside each project.',
      order: 170
    },
    customFooterPath: {
      type: 'string',
      default: join(PACKAGE_NAME, 'footer.js'),
      description: 'You can use this javascript file to customize the document footer. The path is relative to Atom config directory <code>' + getUserAtomPath() + '</code>. This relative path can also be used inside each project.',
      order: 190
    },
    openFile: {
      type: 'boolean',
