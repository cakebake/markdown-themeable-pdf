...
import { get, startsWith, isArray } from 'lodash'
...
export const getMarkdownPreviewGrammars = () => atom.config.get('markdown-preview.grammars')

export const addGrammarToMarkdownPreviewConfig = () => {
  const key = 'markdown-preview.grammars'
  const grammar = 'source.gfm.markdown-themeable-pdf'
  const grammars = getMarkdownPreviewGrammars()
  if (isArray(grammars)) {
    if (grammars.indexOf(grammar) === -1) {
      grammars.push(grammar)
      atom.config.set(key, grammars)
    }
    return getMarkdownPreviewGrammars().indexOf(grammar) !== -1
  }
  return false
}
