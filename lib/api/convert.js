  ...
  return new Promise(async (resolve, reject) => {
    if (!isEmpty(filePath)) {
      try {
        ...
        switch (type) {
          case 'pdf':
            data = await pdf(doc, { ...get(options, 'pdf'), headerTemplate: header(), footerTemplate: footer() })
            break
          case 'jpeg':
            data = await capture(doc, { ...get(options, 'jpeg'), type: 'jpeg' })
            break
          case 'png':
            data = await capture(doc, { ...get(options, 'png'), type: 'png' })
            break
          default:
            data = doc
        }
