...
const markdownToHTML = (markdown, isFinalFormat, options, basePath) => {
  return new Promise((resolve, reject) => {
    try {
      const html = render(markdown, options, isFinalFormat, basePath)
      resolve(html)
    } catch (e) {
      reject(e)
    }
...
const render = (markdown, options, isFinalFormat, basePath) => {
  let md = markdownIt(options)
  if (!isFinalFormat) {
    md = imageSrcManipulation(md, basePath)
...
const imageSrcManipulation = (md, basePath) => {
  ...
  md.renderer.rules.image = (tokens, idx, options, env, self) => {
    ...
    tokens[idx].attrs[index][1] = base64Src(tokens[idx].attrs[index][1], basePath)
