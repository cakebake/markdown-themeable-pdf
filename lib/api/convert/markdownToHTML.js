  ...
  if (get(options, 'enableEmoji', 1) === 1) {
    ...
    md.renderer.rules.emoji = (token, idx) => {
      return parseEmoji(token[idx].content, (icon, options, variant) => {
        const emojiPath = join(dirname(require.resolve('twemoji')), 'svg', `${icon}.svg`)
        const emojiString = readFileSync(emojiPath).toString('base64')
        return `data:image/svg+xml;base64,${emojiString}`
      })
    }
  }
