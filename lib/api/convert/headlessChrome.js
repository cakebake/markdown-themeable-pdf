...
export const pdf = (htmlDocument, options, timeout) => {
  return new Promise(async (resolve, reject) => {
    try {
      const page = await openPage(htmlDocument, timeout)
...
export const capture = (htmlDocument, options, timeout) => {
  return new Promise(async (resolve, reject) => {
    try {
      const page = await openPage(htmlDocument, timeout)
...
const openPage = (htmlDocument, timeout) => {
  return new Promise(async (resolve, reject) => {
    try {
...
export const getWantedRevisionNumber = () => {
  let revision
  try {
    const path = join(dirname(require.resolve('puppeteer')), 'package.json')
    const info = require(path)
    revision = get(info, 'puppeteer.chromium_revision')
    if (revision) {
      return revision
    } else {
      throw new Error(`Could not get chromium revision from ${path}`)
    }
  } catch (e) {
    throw new Error(e)
  }
