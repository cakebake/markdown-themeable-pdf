...
import { get, set, round, isFinite } from 'lodash'
...
/**
 * @return {number} 0 = no download required; 1 = no revision found; [revision] = should update revision;
 */
...
export const deleteOldChromiumRevisions = () => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (localRevisions.length) {
        for (var i = 0; i < localRevisions.length; i++) {
          if (localRevisions[i] !== revision) {
            try {
              await browserFetcher.remove(localRevisions[i])
              removedRevisions.push(localRevisions[i])
            } catch (e) {
              console.error(e)
            }
          }
