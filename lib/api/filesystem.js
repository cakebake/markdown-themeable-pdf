...
import { access, readdirSync, readFile as _readFile, writeFile as _writeFile, existsSync } from 'fs'
...
export const base64Src = (src, baseDir) => {
  if (parse(src).protocol || resolvePath(src) === src) {
    return src
  }
  const resolved = resolvePath(baseDir, src)
  return existsSync(resolved) ? base64Sync(resolved) : src
}
