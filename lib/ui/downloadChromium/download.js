...
const downloadNotification = () => {
  return new Promise(async (resolve, reject) => {
    ...
    let viewTitle
    let revision
    state.notification.download = notification('Downloading Chromium', 'info', {
      dismissable: true
    })
    state.notification.download.onDidDismiss(() => {
      if (!revision) {
    ...
    try {
      try {
        view = atom.views.getView(state.notification.download).element
      ...
      state.notification.download.dismiss()
